...
extension Encodable {
    ...
    func json(encoder: JSONEncoder = .init()) throws -> [String: Any] {
...
extension Encodable {
    ...
    func jsonString(encoder: JSONEncoder = .init(),
                    options: JSONSerialization.WritingOptions = .init()) throws -> String {
        try json(encoder: encoder)
